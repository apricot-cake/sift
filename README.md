...
初期設定は次のとおりです。値は設定画面（拡張機能のアイコンから開きます）、または投稿画面右下の「設定」から変更できます。抽出のON/OFFはホストごとに分かれ、投稿画面の切り替えは表示中のホストだけに反映します。投稿画面の「設定」には、その画面のサービスで使うしきい値だけが出ます。
...
- 画面の言語は英語・日本語・韓国語・繁体字中国語・簡体字中国語・スペイン語・ブラジルポルトガル語の7言語で、ブラウザ自身の言語設定から選ばれます。拡張側に言語の切り替えはありません＝どれでもないブラウザには英語が出ます。
...
いずれの場合も、件数のように数によって言い回しが変わるメッセージは複数形分岐（`1` / `n`。0件だけ別の言い回しが要るなら `0` も）で書きます。書き方は `locales/en.yml` の `toolbarHitCount` などを参照してください。

翻訳の質はここでは担保しません。AIによる下書きも歓迎しますが、その言語のネイティブ、または話者コミュニティによるレビューを経てからマージします。
