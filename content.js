(function initializeContentScript() {
  ...
  const { classifyPost, parseMetric } = globalThis.XIFCore;
  const { defaults, normalizeSettings } = globalThis.XIFSettings;
  ...
  function toolbarMarkup() {
    return `
      ...
      <div class="panel" data-role="panel" hidden>
        <label>
          通常の最低いいね
          <input data-setting="minLikes" type="number" min="0" step="50">
        </label>
        <label>
          上昇中を表示
          <input data-setting="risingEnabled" type="checkbox">
        </label>
        <label>
          上昇中の最低いいね
          <input data-setting="risingMinLikes" type="number" min="0" step="10">
        </label>
        <label>
          投稿後の時間
          <input data-setting="risingMaxAgeHours" type="number" min="1" max="168">
        </label>
        <label>
          メディア
          <select data-setting="mediaMode">
            <option value="any">画像・動画</option>
            <option value="images">画像のみ</option>
          </select>
        </label>
        <label>
          リポストを除外
          <input data-setting="hideReposts" type="checkbox">
        </label>
        <p class="hint">設定は自動保存されます。青線が通常、橙線が上昇中です。</p>
      </div>
      <div class="toolbar">
        <button data-action="toggle-enabled"></button>
        <span class="status" data-role="status">判定中…</span>
        <button data-action="toggle-show-all">全件を一時表示</button>
        <button data-action="toggle-panel">調整</button>
      </div>
    `;
  }
