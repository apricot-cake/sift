...
専用の `--user-data-dir` で Chrome を開きます。通常利用する Chrome とは別プロセスで、同時に利用できます。初回だけ `chrome://extensions` から `~\.sift-dev\chrome-mv3-dev` を読み込み、X にログインしてください。以後はプロファイルが設定を保存します。
