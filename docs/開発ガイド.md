...
複数言語の翻訳は一括して編集します。ロケール変更による拡張機能全体の自動再読み込みは、連続した更新をまとめて一度だけ実行します。

実在するアカウントを使う検証では、検証直前に必要な対象サイトのタブだけを開きます。検証後に通常タブの一覧を取得し、二つ以上ある場合だけ検証タブを閉じます。一つしかない場合は、そのタブを閉じると開発用 Chrome も終了するため残します。

自動読み込みに関わる検証は短時間で打ち切ります。読み込みが連鎖した場合は直ちにフィルターをオフにし、無限読み込みの状態を放置しません。利用者へ確認を戻す前にもフィルターがオフであることを確認します。

実機検証では、ページへダミー投稿や検証用の DOM を追加しません。各プラットフォームが実際に読み込んだ投稿だけを使います。テスト用 HTML を使う自動検査は実機検証の代わりにしません。
...
開発用 Chrome は、背面や他のウィンドウに隠れた状態でも検証対象の描画とタイマーを維持します。画面の表示状態によって実機検証の結果が変わらないようにするためです。
