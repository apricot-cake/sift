...
この文書は、開発環境の準備、拡張機能の起動、検証、配布物の作成を扱います。サービスごとの対象画面と実装上の制約は [対応サービス.md](対応サービス.md) を参照してください。

開発には、通常利用するものとは別の Chrome プロファイルを使います。
...
開発ビルドは、その作業ツリーの `.output\chrome-mv3-dev` に出力します。サーバーは `127.0.0.1:51732` で待ち受けます。ポートが使用中の場合は、別のポートへ移らずに失敗します。拡張機能がこのアドレスを使用するため、同時に起動できる開発サーバーは 1 つです。別の worktree へ切り替える場合は、現在のサーバーを止め、その worktree で起動し直します。
...
専用の `--user-data-dir` で Chrome を開きます。通常利用する Chrome とは別プロセスで、同時に利用できます。初回だけ `chrome://extensions` から `.output\chrome-mv3-dev` を読み込み、X にログインしてください。以後はプロファイルが設定を保存します。worktree を切り替える場合は、展開済み拡張機能の読込先も新しい worktree の同じ相対パスへ切り替えます。

開発用 Chrome は、背面や他のウィンドウに隠れた状態でも検証対象の描画とタイマーを維持します。画面の表示状態によって実機検証の結果が変わらないようにするためです。

検証ツールは `http://127.0.0.1:9222` の CDP に接続します。通常利用するプロファイルや、そのプロファイルに入れた操作用拡張機能は使いません。`npm run dev:browser` が接続失敗を報告した場合は、開発用 Chrome を閉じてから同じコマンドで起動し直してください。

service worker を変更した場合は、`chrome://extensions` で拡張機能をリロードします。開発用プロファイルのウィンドウで `Alt+R` を押してもリロードできます。

開発ビルドとリリースビルドは同じ拡張機能 ID を使うため、同じプロファイルには読み込めません。開発ビルドは専用プロファイルへ読み込んでください。
...
開発ビルドは未捕捉例外を `browser.storage.local` の環状バッファに最大 50 件保存します。service worker がバッファを開発サーバーへ送信し、`~\.sift\extension-errors.log` に JSON Lines 形式で追記します。リリースビルドでは例外を収集しません。

content script は拡張機能が発生元と分かる例外だけを記録します。サイドパネルと service worker はすべての未捕捉例外を記録します。
